...
FROM instructor
WHERE instructor_id NOT IN (SELECT instructor_id FROM student);

-- 4. List students and their instructors for a specific specialization (e.g., 'Science').
SELECT student.student_name, instructor.instructor_name
FROM student
JOIN instructor ON student.instructor_id = instructor.instructor_id
...
SELECT student.student_name, student.student_city, instructor.instructor_name, instructor.instructor_city
FROM student
JOIN instructor ON student.instructor_id = instructor.instructor_id
WHERE student.student_city <> instructor.instructor_city;

-- 6. List All Instructors and Their Students (Including Without Students).
SELECT instructor.instructor_name, student.student_name
FROM instructor
LEFT JOIN student ON instructor.instructor_id = student.instructor_id
...
SELECT instructor.instructor_name, COUNT(student.student_id) AS student_count
FROM instructor
LEFT JOIN student ON instructor.instructor_id = student.instructor_id
WHERE instructor.instructor_city = 'Pune'
GROUP BY instructor.instructor_name;

-- c. List the Number of Students Grouped by Specialization of Instructors in Pune
SELECT instructor.specialization, COUNT(student.student_id) AS student_count
FROM instructor
LEFT JOIN student ON instructor.instructor_id = student.instructor_id
WHERE instructor.instructor_city = 'Pune'
...
FROM instructor
LEFT JOIN student ON instructor.instructor_id = student.instructor_id
WHERE instructor.instructor_city = 'Pune' AND student.student_id IS NULL;

-- Design 5 more queries having join of 3 tables and aggregate functions as per your choice:
-- Note: Since there are only two tables given (student and instructor), the "join of 3 tables" part cannot be fulfilled. 
-- However, I'll provide aggregate functions involving these two tables.

-- 1. Find the number of students each instructor has in each specialization.
SELECT instructor.instructor_name, instructor.specialization, COUNT(student.student_id) AS student_count
FROM instructor
LEFT JOIN student ON instructor.instructor_id = student.instructor_id
GROUP BY instructor.instructor_name, instructor.specialization;

-- 2. List the instructors who have more than 5 students and their specialization.
SELECT instructor.instructor_name, instructor.specialization, COUNT(student.student_id) AS student_count
FROM instructor
LEFT JOIN student ON instructor.instructor_id = student.instructor_id
GROUP BY instructor.instructor_name, instructor.specialization
...
-- 3. Find the total number of students in each city for each specialization.
SELECT instructor.specialization, student.student_city, COUNT(student.student_id) AS total_students
FROM instructor
JOIN student ON instructor.instructor_id = student.instructor_id
GROUP BY instructor.specialization, student.student_city;

-- 4. List instructors along with the total number of students in each city.
SELECT instructor.instructor_name, student.student_city, COUNT(student.student_id) AS total_students
FROM instructor
JOIN student ON instructor.instructor_id = student.instructor_id
GROUP BY instructor.instructor_name, student.student_city;

-- 5. Find the city with the highest number of students for each specialization.
SELECT instructor.specialization, student.student_city, COUNT(student.student_id) AS total_students
FROM instructor
JOIN student ON instructor.instructor_id = student.instructor_id
GROUP BY instructor.specialization, student.student_city
ORDER BY instructor.specialization, total_students DESC;
